...
**This was live for two weeks.** Every file here shipped unsigned from 2026-07-22 until 2026-08-06, so nobody who followed our own instructions could import any of them. Fixed at the same time as the 404 on the old all-in-one Cellular Mode file, which had never been deployed at all. If someone in Discord said the shortcuts "didn't work", they were right and it was not their device.

**That 404 is why the Cellular Mode rename was safe, and it is worth being precise about.** The all-in-one file entered this repo on 2026-08-06 (c2e142c) and had never existed in the tree before that commit — so it 404'd for hours, not weeks, and nothing on the website links to `downloads/shortcuts/` at all (grep for it across `*.html` / `*.js`: no hits). The only route to it was an in-app disclosure shipped in a build from the same day. That is what made the name `Wander Cellular Mode` reusable; the two-week unsigned window is a separate, also-true fact about the other six files.
...
**The filename is not part of the signed bytes** (verified: signing byte-identical content to two different output names produces byte-identical inner payloads). So publishing under the display name costs nothing and invalidates nothing — sign straight to the display name:

```
/usr/bin/shortcuts sign --mode anyone --input unsigned/wander-cellular-mode.shortcut --output "Wander Cellular Mode.shortcut"
```

A space survives end to end: this tree has `.nojekyll` so Pages serves it raw, GitHub sends no `Content-Disposition`, and Safari therefore names the download from the last path component percent-decoded — `Wander%20Cellular%20Mode.shortcut` lands in Files as `Wander Cellular Mode.shortcut`. In-app URL constants must be written **pre-encoded** (`%20`), because `URL(string:)` returns nil on a raw space and the button would silently do nothing.
...
### 1b. Wander Cellular Mode (`Wander Cellular Mode.shortcut`) — the only one Cellular Mode needs

**One import, nothing to edit.** Every action in it is an `is.workflow.actions.*` built-in: an `If` on Shortcut Input, two `Set Airplane Mode` actions, a `Wait`, and an `Open URLs` back to `wander://airplane-ok`. There is no bundle id, no team id and no `AppIntentDescriptor` anywhere in the file, so **one signed copy is correct for the cert build, for every free-sideload re-sign, and for every future signature.**
...
**Why it replaced the all-in-one shortcut.** Wander is the conductor now (`Wander/Services/CellularModeSequence.swift`): it runs this with `"on"`, waits for the cellular and Wi-Fi interfaces to actually disappear, brings the tunnel up and teleports **in-process**, then runs this again with `"off"`. Nothing forced the Shortcut to be the conductor — it only had to be, because it was the thing that could *wait*. Inverting that deletes the two App Intent actions, and with them the setup step that told people to open the Shortcuts editor, which is the step that ended most setups. It also means a failure is now something Wander can put on screen: while a Shortcut ran the sequence unattended, an App Intent's return value was never surfaced at all, so a failed run and a working one looked identical.

Rebuild it with `build-wander-cellular-mode.py` in this directory. That script's docstring records which action identifier and parameter key was verified where. (The script writes to `unsigned/`. It used to write over the published, signed sibling — a plain rebuild silently replaced an AEA1 file with unsigned XML, which iOS refuses outright.)

#### THE NAME IS SHARED WITH A RETIRED FILE — THIS IS THE PART TO READ

Renamed from **Wander Airplane** on 2026-08-06, because the feature is called Cellular Mode in every part of the app and the mismatch was the owner's own complaint. The name was free (see §1a on the 404), but the old all-in-one file was downloadable for a few hours under it, so a handful of libraries hold a *different* shortcut of this exact name.

That matters because `shortcuts://x-callback-url/run-shortcut?name=…` matches a **string**, and iOS gives an app no way to read what it matched. Reaching the old file is not a cosmetic failure: it ignores the input, runs its own sequence, turns Airplane Mode on, and `StartTunnelIntent.openAppWhenRun` foregrounds Wander mid-run — so iOS can suspend it before its (real, unconditional) restore step and the radio stays off.

**The defence is inside the file, not in the app's guesswork.** Its last action opens `wander://airplane-ok`; the retired file's opened `wander://cellular-done`. Disjoint, and both originate *inside* the shortcut — which is the whole point, because Shortcuts fires its own `x-success` for whatever it ran, so identity routed through `x-success` would be forgeable. Then:

- `CellularModeSequence.start` refuses to run until `ShortcutRunner.cellularModeVerified` is true, and only `airplane-ok` from a run Wander invoked **by name** sets it. It fails closed: a user who did not delete the old file cannot arm the feature.
- The setup card's last step is a **check**, not a tick. It runs the shortcut with `"off"` — a no-op on this file — and reports which one answered.
- `wander://cellular-done` is now a tripwire. Receiving it is positive proof the wrong file ran: Wander stops, re-runs the name with `"off"` to get the radio back, and raises a card naming the shortcut to delete.
- Re-running with `"off"` is safe under **either** file while the radio is down. This one takes its `Otherwise` branch; the retired one gates its Airplane-Mode-ON branch on a cellular **Carrier Name**, which does not exist in Airplane Mode, so it falls to its unconditional `Set Airplane Mode → Off` tail. Neither can turn the radio on from there.

**`wander://airplane-ok` is a protocol constant** shared with `MainTabView.handleURL` and `build-wander-cellular-mode.py`. Change it and every installed copy silently stops being recognised.

#### SIGN IT TO ALL FOUR PUBLISHED NAMES

One source, four outputs. The two old "Wander Airplane" paths are kept alive as copies **of this file** — not as redirects and not as their old contents — because builds shipped before the rename link to them, and what those users must not be handed is the retired all-in-one.

```
cd downloads/shortcuts
for out in "Wander Cellular Mode" "wander-cellular-mode" "Wander Airplane" "wander-airplane"; do
...
Every line of that last check must read `41454131`. A build shipped before the rename that downloads the new contents is fine: it sends `wander://airplane-ok`, which its switch has no case for and ignores, and its `x-success` (`wander://open`) still fires.

**BOTH CONDITIONAL FILES WERE BROKEN UNTIL 2026-08-06 AND MUST BE RE-SIGNED.** `WFInput` on an `is.workflow.actions.conditional` is a VARIABLE-typed parameter — `{ "Type": "Variable", "Variable": <token> }` — and both this file and the old all-in-one shipped the bare token. WorkflowKit dropped the parameter silently, so both imported with an **empty condition slot** while every other action rendered fine. Ground truth: 47/47 editor-built If-heads in a real `Shortcuts.sqlite` use the envelope, 0/47 use a bare token. Full write-up in `RECIPES.md`. The other six published files contain no conditionals and are unaffected.

### 1c. The old all-in-one Cellular Mode shortcut — RETIRED 2026-08-06

Deleted from this directory, along with its `unsigned/` source, its in-app fallback UI, its install URL, its readiness flag and its `wander://cellular-missing` callback. Nothing routes to it any more; the only trace left in the app is the tripwire on `wander://cellular-done` described above, which exists precisely so that a leftover copy is **detected** rather than silently run.

**Why retire rather than keep as a fallback.** It was kept as one on the argument that an import can fail for reasons that are not the user's fault, so a second working route was worth a dozen taps in the Shortcuts editor. That argument was written while the whole pack was shipping unsigned and imports genuinely were being refused; every published file now carries the `AEA1` signature. And it inverted once the rename landed: this file is the one thing on a user's phone that can take their signal away by accident, so offering it as a remedy would be handing out the hazard. The escape hatch is now what it always really was — turn Airplane Mode on yourself, teleport, turn it back off — and every failure message in `CellularModeSequence` says so.

**Why it was safe to retire.** It first entered this repo on 2026-08-06 (c2e142c, kebab; 1c32afd, display name), 404'd until that morning, and no page on the site links to `downloads/shortcuts/` — so the only route to it was an in-app disclosure shipped the same day, behind three taps and a dozen editor operations. The reachable population is effectively the owner.

**One correction worth keeping, because it was believed and repeated in code comments.** Its restore step was REAL: action 15, `is.workflow.actions.airplanemode.set` with `OnValue = False`, unconditional, outside every If-group, followed by `Open URLs → wander://cellular-done`. Its own comment said the tail "always run[s], on every path". Several app-side comments asserted "the shortcut no longer turns Airplane Mode back off"; that described an intermediate design the shipped file never matched, and it sent a later change looking for the wrong bug. Its real hazard is re-entrancy — `openAppWhenRun` foregrounding Wander mid-run — not a missing action.

Its full action-by-action breakdown lived here and in `RECIPES.md`; both are gone with it. `git log` has them if the reasoning is ever needed again.
